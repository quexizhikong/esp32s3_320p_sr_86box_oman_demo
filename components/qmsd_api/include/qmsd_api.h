...
///* qmsd base */
#define D_SMART_TRIGGERS   	"triggers"//调光分组/一键执行/自动化的条件
#define D_SMART_TODO   		"todo"//调光分组/一键执行/自动化的执行
#define D_DEVICE_FILE   	"devicefile"//设备信息文件夹
#define D_LIGHT_GROUP   	"light_group"//调光分组文件夹

/*!< qmsd nvs */
//参数说明
//name:nvs的命名空间
//key:nvs的键
//data/value/input/output是内容
//length/size/blob_size/str_size是长度
...
esp_err_t qmsd_nvs_set_u8(const char *name, const char *key, uint8_t value);
esp_err_t qmsd_nvs_get_u8(const char *name, const char *key, uint8_t *value);
esp_err_t qmsd_nvs_set_i8(const char *name, const char *key, int8_t value);
...
esp_err_t qmsd_nvs_clear_namespace(const char*name);										//清除命名空间
esp_err_t qmsd_nvs_clear_key(const char*name, const char* key);								//根据命名空间,清除key
esp_err_t qmsd_nvs_find_key(const char*name, const char* key); 								//根据命名空间,找key
esp_err_t qmsd_nvs_print_find_key(const char*name);											//根据命名空间,遍历key
