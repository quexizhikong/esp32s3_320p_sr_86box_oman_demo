...
static gsln_POINT_TYPE point_array[POINT_DEEP][POINT_MAX];
static gsln_POINT_TYPE *point_pointer[PP_DEEP];
static gsln_POINT_TYPE *point_stretch[PS_DEEP];
static gsln_POINT_TYPE *point_report[PR_DEEP];
static gsln_POINT_TYPE point_now[POINT_MAX];
static gsln_DELAY_TYPE point_delay[POINT_MAX];
static int filter_deep[POINT_MAX];
static int avg[AVG_DEEP];
static gsln_EDGE_TYPE point_edge;
static gsln_DECIMAL_TYPE point_decimal[POINT_MAX];
...
static gsln_STATE_TYPE global_state;
...
static gsln_FLAG_TYPE global_flag;
static gsln_ID_FLAG_TYPE id_flag;
...
static gsln_POINT_TYPE point_press_move;
...
static void PointPredict(void)
{
    ...
    for (i = 0; i < POINT_MAX; i++)
    {
        if (pp[1][i].all != 0)
        {
            ...
            else if (global_state.other.interpolation || pp[2][i].all == 0 || pp[2][i].other.fill != 0 || pp[3][i].other.fill != 0 || pp[1][i].other.key != 0 || global_state.other.only)
            {
                PointPredictOne(i);
            }
...
static unsigned int PointDistance(gsln_POINT_TYPE *p1, gsln_POINT_TYPE *p2)
{
    int a, b, ret;
...
static void DistanceInit(gsln_DISTANCE_TYPE *p)
{
    int i;
...
static int DistanceMin(gsln_DISTANCE_TYPE *p)
{
    int i, j;
    p->min = 0x7fffffff;
...
static void DistanceIgnore(gsln_DISTANCE_TYPE *p)
{
    int i, j;
    for (i = 0; i < POINT_MAX; i++)
    ...
    for (j = 0; j < POINT_MAX; j++)
    {
        for (i = j + 1; i < POINT_MAX; i++)
        {
            if (pp[0][i].all == 0 || pp[0][j].all == 0 || pp[1][i].all == 0 || pp[1][j].all == 0)
                continue;
            if (((pp[0][j].other.x < pp[0][i].other.x && pp[1][j].other.x > pp[1][i].other.x) || (pp[0][j].other.x > pp[0][i].other.x && pp[1][j].other.x < pp[1][i].other.x)) && ((pp[0][j].other.y < pp[0][i].other.y && pp[1][j].other.y > pp[1][i].other.y) || (pp[0][j].other.y > pp[0][i].other.y && pp[1][j].other.y < pp[1][i].other.y)))
            {
                t = pp[0][i].all;
...
static void GetPointNum(gsln_POINT_TYPE *pt)
{
    int i;
    point_num = 0;
    for (i = 0; i < POINT_MAX; i++)
...
static void PointMenu(void)
{
    ...
    {
        if ((pp[0][0].other.x < edge_e && pp[1][0].other.x < edge_e) || (pp[0][0].other.y < edge_e && pp[1][0].other.y < edge_e) || (pp[0][0].other.x > drv_num_nokey * 64 - edge_e && pp[1][0].other.x > drv_num_nokey * 64 - edge_e) || (pp[0][0].other.y > sen_num_nokey * 64 - edge_e && pp[1][0].other.y > sen_num_nokey * 64 - edge_e))
        {
...
static unsigned int KeyMap(int *drv, int *sen)
{
    ...
    for (i = 0; i < 8; i++)
    {
        if ((unsigned int)*drv >= (key_range[i].up_down >> 16) && (unsigned int)*drv <= (key_range[i].up_down & 0xffff) && (unsigned int)*sen >= (key_range[i].left_right >> 16) && (unsigned int)*sen <= (key_range[i].left_right & 0xffff))
        {
...
static unsigned int ScreenResolution(gsln_POINT_TYPE *p)
{
    int x, y;
    x = p->other.x;
    y = p->other.y;
...
static void PointReport(struct gsln_touch_info *cinfo)
{
    int i;
...
static void PointEdge(void)
{
    ...
    for (i = 0; i < 4; i++)
    {
        if (id_flag.other.screen_core)
            break;
        if (stretch->right[i].range > screen_y_max * 64 / 128 || stretch->down[i].range > screen_x_max * 64 / 128 || id_flag.other.screen_real)
        {
            for (i = 0; i < 4; i++)
            {
                if (stretch->up[i].range)
...
static void ResetMask(void)
{
    ...
    else if (PointDistance((gsln_POINT_TYPE *)(&reset_mask_max), pp[0]) > (((unsigned int)reset_mask_dis) & 0xffffff) && reset_mask_count > (((unsigned int)reset_mask_dis) >> 24))
...
static int DiagonalDistance(gsln_POINT_TYPE *p, int type)
{
    int divisor, square;
...
static void DiagonalCompress(gsln_POINT_TYPE *p, int type, int dis, int dis_max)
{
    int x, y;
    int tx, ty;
    int cp_ceof;
...
static void PointDiagonal(void)
{
    ...
    for (i = 0; i < POINT_MAX; i++)
    {
        ...
        {
            if ((ps[0][i].other.x <= diagonal_start && ps[0][i].other.y <= diagonal_start) || (ps[0][i].other.x >= drv_num_nokey * 64 - diagonal_start && ps[0][i].other.y >= sen_num_nokey * 64 - diagonal_start))
                point_corner |= 0x2 << i * 2;
            else if ((ps[0][i].other.x <= diagonal_start && ps[0][i].other.y >= sen_num_nokey * 64 - diagonal_start) || (ps[0][i].other.x >= drv_num_nokey * 64 - diagonal_start && ps[0][i].other.y <= diagonal_start))
                point_corner |= 0x3 << i * 2;
            else
...
static void PressMove(void)
{
    ...
    else if (point_press_move.other.x && point_press_move.other.y)
    {
        if (PointDistance(&point_press_move, &pr[0][0]) > (press_move >> 16) * (press_move >> 16))
        {
            ...
            // 				press_move = 4;
            if (pr[0][0].other.x < point_press_move.other.x && pr[0][0].other.y < point_press_move.other.y + (point_press_move.other.x - pr[0][0].other.x) && pr[0][0].other.y + (point_press_move.other.x - pr[0][0].other.x) > point_press_move.other.y)
                point_press_move.all = 1;
            else if (pr[0][0].other.x > point_press_move.other.x && pr[0][0].other.y < point_press_move.other.y + (pr[0][0].other.x - point_press_move.other.x) && pr[0][0].other.y + (pr[0][0].other.x - point_press_move.other.x) > point_press_move.other.y)
                point_press_move.all = 2;
            else if (pr[0][0].other.y < point_press_move.other.y && pr[0][0].other.x < point_press_move.other.x + (point_press_move.other.y - pr[0][0].other.y) && pr[0][0].other.x + (point_press_move.other.y - pr[0][0].other.y) > point_press_move.other.x)
                point_press_move.all = 3;
            else if (pr[0][0].other.y > point_press_move.other.y && pr[0][0].other.x < point_press_move.other.x + (pr[0][0].other.y - point_press_move.other.y) && pr[0][0].other.x + (pr[0][0].other.y - point_press_move.other.y) > point_press_move.other.x)
...
static void gsln_id_reg_init(int flag)
{
    int i, j;
...
void gsln_DataInit(unsigned int *conf_in)
{
    int i, j;
    unsigned int *conf;
    int len;
...
void gsln_alg_id_main(struct gsln_touch_info *cinfo)
{
    int i;
