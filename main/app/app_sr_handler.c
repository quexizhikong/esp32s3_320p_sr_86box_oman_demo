...
static esp_err_t sr_echo_play(audio_segment_t audio)
{
  //  bsp_board_power_ctrl(POWER_MODULE_AUDIO, false); // turn off the speaker to avoid play some noise
    ...
    if (NULL == strstr((char *)wav_head->Subchunk1ID, "fmt") &&
    	...
    	ESP_LOGI(TAG, "Header of wav format error");
        return ESP_FAIL;
    }
    ...
    float sample_rate=i2s_get_clk(BOARD_I2S_NUM);
    ESP_LOGI(TAG, "defalt sample_rate:%d",(uint32_t)sample_rate);

    i2s_zero_dma_buffer(BOARD_I2S_NUM);
   // i2s_set_clk(BOARD_I2S_NUM, wav_head->SampleRate, wav_head->BitsPerSample, wav_head->NumChannels);
    vTaskDelay(pdMS_TO_TICKS(50));
   // bsp_board_power_ctrl(POWER_MODULE_AUDIO, true);// turn on the speaker
    size_t bytes_written = 0;
    b_audio_playing = true;
    i2s_write(BOARD_I2S_NUM, p, len, &bytes_written, portMAX_DELAY);
    i2s_zero_dma_buffer(BOARD_I2S_NUM);
...
void sr_handler_task(void *pvParam)
{
    ...
    for (size_t i = 0; i < AUDIO_MAX; i++)
    {
        ...
        ESP_LOGI(TAG, "audio_file-->%s,file_size-->%d",audio_file,file_size);
...
//            }
            char evt;
            evt=3;
            xQueueSend(custom_rx_evt_queue, &evt, 1000 / portTICK_PERIOD_MS);
...
//            }
        ...
        {
            ESP_LOGI(TAG, "command:%d", result.command_id);
